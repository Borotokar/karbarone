    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Token` (`id` INTEGER NOT NULL, `token` TEXT NOT NULL, PRIMARY KEY (`id`))');
...
class _$TokenDao extends TokenDao {
  _$TokenDao(
    ...
    this.changeListener,
        ...
        _tokenInsertionAdapter = InsertionAdapter(
            ...
            'Token',
            (Token item) =>
                <String, Object?>{'id': item.id, 'token': item.token}),
  ...
  @override
  Future<Token?> findTokenById(int id) async {
    return _queryAdapter.query('SELECT * FROM Token WHERE id = ?1',
        mapper: (Map<String, Object?> row) =>
            Token(row['id'] as int, row['token'] as String),
        arguments: [id]);
  }

  @override
  Future<String?> getToken() async {
    return _queryAdapter.query('SELECT token FROM Token WHERE id = 1 LIMIT 1',
  ...
  Future<void> deleteToken(Token token) async {
    await _tokenDeletionAdapter.delete(token);
  }
}
